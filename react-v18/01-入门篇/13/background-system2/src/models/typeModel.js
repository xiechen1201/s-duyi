// 请求方法
import { addTypeApi, deleteTypeApi, getTypeApi } from '@/services/type';
...
export default {
  ...
  effects: {
    ...
    *_initTypeList(_, { put, call }) {
      // 从服务器获取数据
      const { data } = yield call(getTypeApi);
      // 调用 reducer 方法更新本地仓库
    ...
    *_addType({ payload }, { put, call }) {
      // 和服务器通信，进行新增
      const { data } = yield call(addTypeApi, payload);
      // 调用 reducer 方法更新本地仓库
    ...
    *_deleteType({ payload }, { put, call }) {
      // 和服务器通信，进行删除
      yield call(deleteTypeApi, payload._id);
      // 本地仓库也需要同步更新
